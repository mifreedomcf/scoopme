...
// ---- Milestone 4: guardian and minor architecture (flag-disabled) ----

export const manageDependentProfile = (payload: Record<string, unknown>) =>
  callFunction("manage-dependent-profile", payload);

export const verifyGuardianAuthority = (payload: Record<string, unknown>) =>
  callFunction("verify-guardian-authority", payload);

export const requestMinorConsent = (rideRequestId: string, scope?: "single_ride" | "standing") =>
  callFunction("request-minor-consent", { ride_request_id: rideRequestId, scope });

export const signMinorConsent = (payload: Record<string, unknown>) =>
  callFunction("sign-minor-consent", payload);

export const revokeConsent = (consentRecordId: string) =>
  callFunction("revoke-consent", { consent_record_id: consentRecordId });

export const verifyMinorHandoff = (payload: Record<string, unknown>) =>
  callFunction("verify-minor-handoff", payload);

export const postMessage = (threadId: string, body: string) =>
  callFunction("post-message", { thread_id: threadId, action: "post", body });

export const readMessages = (threadId: string) =>
  callFunction("post-message", { thread_id: threadId, action: "read" });

// ---- Milestone 5: location and communication ----

export const startLocationSharing = (rideRequestId: string, audiences?: string[]) =>
  callFunction("start-location-sharing", { ride_request_id: rideRequestId, audiences, acknowledged: true });

export const stopLocationSharing = (rideRequestId: string) =>
  callFunction("stop-location-sharing", { ride_request_id: rideRequestId });

export const recordLocationPing = (rideRequestId: string, latitude: number, longitude: number, accuracy?: number) =>
  callFunction("record-location-ping", {
    ride_request_id: rideRequestId, latitude, longitude, accuracy_meters: accuracy,
  });

export const readTracking = (token: string) => callFunction("read-tracking", { token });

export const openRelaySession = (rideRequestId: string) =>
  callFunction("open-relay-session", { ride_request_id: rideRequestId });
