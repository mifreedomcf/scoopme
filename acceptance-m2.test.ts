...
describe("scheduled automations", () => {
  ...
  it("ships every scheduled job with valid automation configuration", () => {
    // Milestone 5 adds the nightly retention sweep to this set.
    expect(configs.map((c) => c.name).sort()).toEqual([
      "credential-expiry-sweep", "dispatch-notifications", "retention-sweep", "ride-checkin-sweep",
    ]);
