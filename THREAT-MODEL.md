...
## T23 — A tracking link is forwarded or guessed

*Someone passes a live link on, or tries to enumerate tokens.*

Mitigated: tokens are 256 bits of randomness, never derived from an id, minted
one per audience and never reused. A token is scoped to one ride and one
audience, and the audience must be one the grantor actually agreed to. It
expires with the trip plus ten minutes, capped at eight hours, and is rate
limited to thirty reads a minute. Every read is audited with the audience and
whether it was precise. An invalid token and a revoked one return the same
answer, so probing reveals nothing. Withdrawing consent revokes every token
immediately rather than letting them run to expiry.
Residual: a link forwarded during the trip works until it expires. The audit
trail shows the read volume, and the grantor can kill it in one tap.

## T24 — Location outlives its purpose

*Coordinates sit in the database long after the ride, waiting to be subpoenaed,
breached, or browsed.*

Mitigated: `LocationEvent` has the shortest retention of anything in the system —
seven days by default, and it is the first rule in the schedule. Exact points
stop being served the moment a ride leaves its active states, regardless of
retention. Coordinates carry field-level security, never appear in any report,
export, or notification, and `reporting.ts` contains no reference to them at all.
Residual: seven days is a default, not a reviewed figure. Setting it deliberately
is part of the `privacy_security` gate.

## T25 — The deletion job destroys evidence

*A retention sweep removes records that an incident or a lawsuit needed.*

Mitigated: the sweep is off until an administrator turns it on. It reads the hold
table once and, if it cannot, deletes nothing at all. A hold on a record or on a
whole entity beats the schedule. `AuditLog`, `SafetyIncident`, `ConsentRecord`,
`LegalAcceptance` and `DataRetentionHold` are on a never-swept list that an admin
override cannot modify. Deletes are soft, so a hard purge remains a deliberate
human act. Anonymization is idempotent and keeps the row.

## T26 — A driver keeps a rider's phone number

Partially mitigated: with a relay configured, neither party ever sees the other's
number. Without one, an adult rider's number is shown only inside the two-hour
reveal window, with the driver told not to save it — and a child's ride never
exposes a number at all, whatever the provider situation.
Residual: a number shown once can be written down. The relay provider is the real
fix and is an open dependency.
